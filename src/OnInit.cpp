...
bool CApp::OnInit() {
  ...
  _translationSpeed = 0.;
  _background1_speed = 5;
  _background2_speed = 10;
  _background3_speed = 15;
  ...
  // reserve stuff
  Entity::EntityList.reserve(5000);
  Entity::NextEntityList.reserve(5000);
  Entity::EntityListToRemove.reserve(5000);
  ...
  Camera::CameraControl.SetTarget(_center);
  Camera::CameraControl.setParent(this);

  if(Area::AreaControl.OnLoad("./maps/1.area") == false) {
    return false;
  }
  if(Area::Background1.OnLoad("./maps/back1.area") == false) {
    return false;
  }
  if(Area::Background2.OnLoad("./maps/back1.area") == false) {
    return false;
  }
  ...
  //int keyB = Entity::CurrentEntityId;
